...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ...
    Scroll_PatStrip:    .dsb 54  ; $0382-$03B7 (may be less?) This stores a strip of 8x8 blocks, (non-vertical level: top to bottom with 2 columns), to render the next 16 pixel "sliver" of screen space
    Scroll_ToVRAMHA:    .dsb 1   ; High byte for when pushing attribute data to VRAM (Set to $23, attribute table 0)
    Scroll_LastAttr:    .dsb 1
    Scroll_AttrStrip:   .dsb 14  ; $03BA-$03C7 (may be less?) This stores a 2x2 block of attribute coloring data
    World_Num_Debug:    .dsb 1   ; When debug mode is activated (KKKZSPIU), this is the world you select to start on
    Map_StarsDeltaX:    .dsb 1   ; Delta the stars move in X to reach the Player (always positive, code figures direction)
    Map_StarsDeltaY:    .dsb 1   ; Delta the stars move in Y to reach the Player (always positive, code figures direction)

                .dsb 16  ; $03CB-$03DA unused

    Map_Stars_PRelX:    .dsb 1   ; During world intro, screen relative position of Player X
    Map_Stars_PRelY:    .dsb 1   ; During world intro, screen relative position of Player Y
    Player_Power:       .dsb 1   ; >>>>>>[P] charge level ($7F max)
    ...
    Level_JctFlag:      .dsb 1   ; Toggles when you junction

                .dsb 1   ; $03E0 unused

    Map_DrawPanState:   .dsb 1   ; Map draw/pan state
    ObjGroupRel_Idx:    .dsb 1   ; Holds relative index of object within its group (see PRG000_CA51)
    InvFlip_VAddrHi:    .dsb 1   ; Hi byte of VRAM address during inventory flip modifications
    ...
    InvFlip_Frame:      .dsb 1   ; Sort of a "frame" of animation during flipping of the inventory box; 0-7 during closing, 8-15 during opening
    InvFlip_Counter:    .dsb 1   ; 0 = Closed, 4 = Fully Open
    InvStart_Item:      .dsb 1   ; Starting inventory item; typ. $00, $07, $0E, $15 (Rows 1-4)
    InvHilite_X:        .dsb 1   ; Current hilite position X coordinate
    InvHilite_Item:     .dsb 1   ; Which item in the current row is highlighted, 0-6

    THouse_ID:      .dsb 1   ; An unused ID value that would track boxes already opened in a Toad House (concept unused!)
    ...
    ; 6 = Random Super Suit
    ; 7 = Random Basic Item
    THouse_Treasure:    .dsb 1

    Coins_Earned:       .dsb 1   ; A "buffer" of coins earned to be added to your total, actual coinage stored in Inventory_Coins[2]
    Map_Powerup_Poof:   .dsb 1   ; Counter that handles the "poof" effect when a powerup is used on the map (requires Inventory to be open, and forces it to close afterward)
    ...
    Level_FreeVertScroll:       .dsb 1

    Level_7Vertical:        .dsb 1   ; Set in World 7 vertical type levels
    Level_SelXStart:        .dsb 1   ; Selects X starting position when level begins (valid values 0-3)
...
UPDATERASTER_32PIXSHOWSPR= $80  ; If NOT set, hides sprites that fall beneath the partition (i.e. for fixed water effect)
    Update_Request:     .dsb 1   ; This changes the current Raster_Effect and Update_Select and doesn't persist
    Map_Starman:        .dsb 1   ; Player used a Starman!
    Map_Power_Disp:     .dsb 1   ; This is the powerup currently DISPLAYED on the map; it should be the same as $0746 World_Map_Power, except for Judgem's Cloud
...
; W8D = World 8 Darkness; overlaps the vars used by the entrance transition
    Map_W8D_VAddrH:     .dsb 1
                ...
                .dsb 3   ; $0448-$044A unused

    Map_W8D_VAddrL:     .dsb 1

                .dsb 5   ; $044C-$0450 unused

    Map_W8D_VAddrH2:    .dsb 1

                .dsb 1   ; $0452 unused

    Map_W8D_VAddrL2:    .dsb 1

                .dsb 1   ; $0454 unused

    Map_W8D_TileOff:    .dsb 1   ; In-tile offset (0 = upper left, 1 = lower left, 2 = upper right, 3 = lower right)

                .dsb 2   ; $0456-$0457 unused

    Map_W8D_YOff:       .dsb 1   ; Y Offset from Player when doing darkness
    Map_W8D_XOff:       .dsb 1   ; X Offset from Player when doing darkness
    Map_W8D_RC:     .dsb 1   ; Row in the upper bits, column in the lower bits
    Map_W8D_Dir:        .dsb 1   ; Direction of travel in darkness (1 = Right, 2 = Left, 4 = Down, 8 = Up)
    Map_W8D_X:      .dsb 1
    Map_W8D_Y:      .dsb 1
    Map_W8D_Idx:        .dsb 1
    ...
    Map_EntTran_VLHalf: .dsb 1   ; When 1, offset starts vertically "lower half" (at $F0 of first screen)
    Map_EntTran_TBCnt:  .dsb 1   ; decreasing counter as the transition moves inward top/bottom
    Map_EntTran_LRCnt:  .dsb 1   ; decreasing counter as the transition moves inward left/right
    Map_EntTran_BVAddrH:    .dsb 4   ; $0447-$044A Each border's current high part of VRAM address
    Map_EntTran_BVAddrL:    .dsb 4   ; $044B-$044E Each border's current low part of VRAM address
    Map_EntTran_BorderLoop: .dsb 1   ; Border loop counter
    Map_EntTran_Cnt:    .dsb 1
    Map_EntTran_VAddrH: .dsb 1   ; high part of VRAM address to modify
    Map_EntTran_VAddrHAdj:  .dsb 1   ; An adjusted version of the high address as needed to wrap (used in removed "box out" version only)
    Map_EntTran_VAddrL: .dsb 1   ; low part of VRAM address to modify
    Map_EntTran_TileOff:    .dsb 1   ; Offset into tile memory (used in removed "box out" version only)
    Map_EntTran_Tile8x8:    .dsb 1   ; Offset to which 8x8 pattern of the tile we're grabbing that we need (used in removed "box out" version only)
    Map_EntTran_VRAMGap:    .dsb 1   ; Sets gap (i.e. 1 for vertical, 32 for horizontal; used in removed "box out" version only)

                .dsb 8   ; $0457-$045E unused

    Map_EntTran_Temp:   .dsb 1   ; Seems to me a multi-purpose value in entrance transition
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ...
    Roulette_PosHi:     .dsb 3   ; $0403-$0405 Hi part of Roulette_Pos
    Roulette_ShapeLock: .dsb 3   ; $0406-$0408 Locked position of each row (0 = Mushroom, 1 = Flower, 2 = Mushroom, 3 = Star)
    Roulette_ConfigRun: .dsb 1   ; 0 = Configuring, 1 = Running
    Roulette_ConfigState:   .dsb 1   ; Early configuration state
    Roulette_RunState:  .dsb 1   ; Running state of game

    ; NOTE: Watch the reuse here...
    Roulette_StopState:     ; $040C-$040E Current "state" of each row while it is coming to a stop (1+)
    Roulette_xUpY:      .dsb 1   ; "x Up" display Y position (overlaps first byte of Roulette_StopState)
    Roulette_LivesToGive:   .dsb 1   ; Lives left to give in reward (overlaps second byte of Roulette_StopState)
                .dsb 1   ; Third byte of Roulette_StopState

    Roulette_Speed:     .dsb 3   ; $040F-$0411 Movement speed of each row (4.4FP)
    Roulette_StopCnt:   .dsb 3   ; $0412-$0414 Decrements to zero while each row is stopping
    Roulette_PosFrac:   .dsb 3   ; $0415-$0417 Fractional accumulator for position of row 1-3

                .dsb 1   ; $0418 unused

    Roulette_Turns:     .dsb 1   ; UNUSED "Turns Remaining" (if > 0, and you lose Roulette, hit a button and spin again!)

                .dsb 12      ; $041A-$0425 unused
    ...
    Bonus_PrizeX:       .dsb 1   ; UNUSED Prize sprite X

    Card_SelectY:           ; Card cursor Y (NOTE: Shared with Bonus_PrizeY)
    Bonus_PrizeY:       .dsb 1   ; UNUSED Prize sprite Y

    Card_Index:         ; Currently selected card in "N-Spade" Card game (NOTE: Shared with Bonus_CurBufOffset)
    Bonus_CurBufOffset: .dsb 1   ; Offset in graphics buffer while generating instruction box

    Card_FirstIndex:    .dsb 1   ; Card_Index of the first card you flipped (in case you get it wrong on the second...)

    Bonus_Round2:           ; UNUSED Picks which "Round 2" game would be played (NOTE: Shared with Card_SelectXOrig)
    Card_SelectXOrig:   .dsb 1   ; Card cursor original X

    Card_MoveDelay:     .dsb 1   ; Decrements to zero; if not zero, can't move on N-Spade game
    Card_SprRAMOff:     .dsb 1   ; Sprite RAM offset after cursor is drawn; for drawing card flip
    Card_AnimTick:      .dsb 1   ; Animation tick, decrements to zero
    Card_VRAM_L:        .dsb 1   ; Low part of VRAM address of N-Spade card to modify
    Card_VRAM_H:        .dsb 1   ; High part of VRAM address of N-Spade card to modify

    Card_FlipCount:     .dsb 1   ; +1 for every card flip
    Card_MatchCard:     .dsb 1   ; Card to match, i.e. the first card you selected of the pair
    Card_UnusedVL:      .dsb 1   ; ?? Some VRAM Low; Seems to only be part of an unused routine
    Card_UnusedVH:      .dsb 1   ; ?? Some VRAM High; Seems to only be part of an unused routine
    ...
    Card_GameState:     .dsb 1   ; State of N-Spade card game
    Card_TurnsRemain:   .dsb 1   ; Number of turns remaining
    Card_FlipFrame:     .dsb 1   ; Frame of flipping card

; The next three vars belong to an unused routine @ PRG022:DA62; unclear what it did exactly
    Card_UnusedArray:   .dsb 4   ; $0437-$043A ??
    Card_UnusedAttr:    .dsb 1   ; Seems to only be part of an unused routine; attribute table offset of some sort
    Card_UnusedArrIdx:  .dsb 1   ; Seems to only be part of an unused routine; index for Card_UnusedArrIdx

    Card_Matches:       .dsb 1   ; Matches (local, this round; Seems to only be used as part of the unused routine)
    Card_CoinsToGive:   .dsb 1   ; If greater than zero, coins are being given
    ...
    ; 9 = Exiting
    Bonus_GameState:    .dsb 1

    Card_InitState:     .dsb 1   ; Initialization state of N-Spade card game
    ...
    ; 2 = N-Spade (UNUSED BONUS GAMES NOTE: May have once been the "even" game spoken of in type 5)
        ; 3:        .dsb 1   ; I suspect this is a placeholder ('C' being the 3rd)
        ; 4:        .dsb 1   ; I suspect this is a placeholder ('D' being the 4th)
...
BONUS_UNUSED_2RETURN    = 7 ; MAY have been Koopa Troopa's "Prize" Game...
    Bonus_GameType:     .dsb 1

    Bonus_KTPrize:      .dsb 1   ; UNUSED Koopa Troopa's "Prize" Game Prize ID (0 = Mushroom, 1 = Star, 2 = Flower, 3 = Judgem's, by BMF54123's patch)
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ...
    Fade_State:     .dsb 1   ; 0 = Nothing, 1 = Fade in, 3 = Fade out
    Fade_Tick:      .dsb 1   ; Ticks down and then decrements Fade_Level
    Fade_Level:     .dsb 1   ; 4 to 0, fade in level
    FadeOut_Cancel:     .dsb 1   ; If set, the next attempted fade out will be cancelled, which then resets this to zero
    Player_AllowAirJump:    .dsb 1   ; Counts down to zero, but while set, you can jump in the air
    Player_XVelAdj:     .dsb 1   ; Applies additional value to the X Velocity

    CineKing_Frame:         ; King's animation frame (NOTE: Shared with Objects_Var7 first byte)
    Objects_Var7:       .dsb 8   ; $0421-$0428 General object variable 7
...
; Lower ranges are context-dependent
    ...
    Music_TriTrkPos:    .dsb 1   ; Offset of triangle track in currently playing index
    Music_NseTrkPos:    .dsb 1   ; Offset of noise track in currently playing index
    Music_PCMTrkPos:    .dsb 1   ; Offset of DMC track in currently playing index
    Music_Sq2RestH:     .dsb 1   ; Square 2 Track hold for rest values to be applied after each event
    Music_Sq2Rest:      .dsb 1   ; Square 2 Track "Rest" period (counts down to zero)
    Music_Sq2NoteLen:   .dsb 1   ; Square 2 Track length of note (counts down to zero, cuts off sound)
    Music_Sq1Rest:      .dsb 1   ; Square 1 Track "Rest" period (counts down to zero)
    Music_Sq1NoteLen:   .dsb 1   ; Square 1 Track length of note (counts down to zero, cuts off sound)
    Music_TriRestH:     .dsb 1   ; Triangle Track hold for rest values to be applied after each event
    Music_TriRest:      .dsb 1   ; Triangle Track "Rest" period (counts down to zero)
    Music_NoiseRest:    .dsb 1   ; Noise Track "Rest" period (counts down to zero)
    Music_NseRestH:     .dsb 1   ; Noise Track hold for rest values to be applied after each event
    Music_DMCRest:      .dsb 1   ; DMC Track "Rest" period (counts down to zero)
    Music_DMCRestH:     .dsb 1   ; DMC Track hold for rest values to be applied after each event
    Music_PCMStart:     .dsb 1   ; Holds the starting offset of the DMC track
    Music_NextIndex:    .dsb 1   ; Next "index" to be played

    SFX_Counter1:       .dsb 1   ; Generic purpose SFX counter
    SndCur_Player:      .dsb 1   ; Currently playing Player sound (blocks music on Square 1)
    SndCur_Level1:      .dsb 1   ; Currently playing "level 1" sound (blocks music on Square 2)
    SndCur_Level2:      .dsb 1   ; Currently playing "level 2" sound
    SndCur_Music1:      .dsb 1   ; Currently playing BGM from Music 1 set
    SndCur_Music2:      .dsb 1   ; Currently playing BGM from Music 2 set
    SndCur_Map:     .dsb 1   ; Currently playing map sound (blocks music on Square 1)
    SndCur_Pause:       .dsb 1   ; Holds copy of Sound_QPause, to play sound while everything else is paused
    SFX_Counter2:       .dsb 1   ; Generic purpose SFX counter for level sounds
    SFX_Counter3:       .dsb 1   ; Generic purpose SFX counter
    SFX_Counter4:       .dsb 1   ; Generic purpose SFX counter

                .dsb 1   ; $04EB unused
                .dsb 1   ; $04EC unused

    Sound_IsPaused:     .dsb 1   ; When set, sound processing is PAUSED
...
PAUSE_RESUMEMUSIC   = $02   ; Resume sound (resumes music)
    ...
    DMC_Time:       .dsb 1   ; Time remaining on DMC sound
    Music_Sq1RestH:     .dsb 1   ; Square 1 Track hold for rest values to be applied after each event
    Music_Sq1AltRamp:       .dsb 1   ; When Square 1 track encounters a $00 byte, it actives a ramping mode, the value of which is stored here
    Music_LOST4FB:      .dsb 1   ; AFAIK, value in music engine that is "lost"; nothing seems to set it, and it doesn't do very much (possibly was a note length override??)
    Music_LOST4FC:      .dsb 1   ; AFAIK, value in music engine that is "lost"; nothing seems to set it, and it doesn't do very much

    Music_RestH_Base:   .dsb 1   ; Base offset into Music_RestH_LUT
    Music_Sq2TrkOff:    .dsb 1   ; Offset of square wave 2 track in currently playing index
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ...
    Title_MLAccelCnt:   .dsb 1   ; constant moving counter which overflows to provide a more "natural" acceleration to the Bros
    Title_MarioPoof:    .dsb 1   ; countdown of Mario's "poof" when he collects the power up
    Title_MLHoldTick:   .dsb 1   ; Set to $40 when Mario/Luigi are "held" (count down to zero)
    Title_ObjVar:       .dsb 6   ; $513-$518 Minor objects user-defined variable 1 (automatically decrements if not zero!)
    Title_ObjVar2:      .dsb 6   ; $519-$51E Minor objects user-defined variable 2
    Title_ObjFrame:     .dsb 6   ; $51F-$524 Minor object "frame"; adds 2x the value here to the pattern selection for the object's sprites
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ...
    Map_Object_ActY:    .dsb 15  ; $0500-$050E For map objects, their actual display Y coordinate (as it may modify, esp. with marchers)
    Map_Object_ActX:    .dsb 15  ; $050F-$051D For map objects, their actual display X coordinate (as it may modify, esp. with marchers)
    Map_Object_ActXH:   .dsb 15  ; $051E-$052C For map objects, their actual display X Hi byte (as it may modify, esp. with marchers)
    Map_Object_Data:    .dsb 15  ; $052D-$053B For marching objects, 0/1 for their direction, could be anything though
    Map_March_Count:    .dsb 15  ; $053C-$054A Simply counts and overflows, but used to determine position of marching map objects (PER ID, not object index ... Actually is seems they go back and forth on that??)

                .dsb 60  ; $054B-$0586 unused

    Map_Objects_Vis:    .dsb 15  ; $0587-$058E Set for map objects as visible, clear if it's not
    Map_MarchInit:      .dsb 1   ; Set when marching data has been initialized (done once per marching cycle on the map)
    Map_InCanoe_Flag:   .dsb 1   ; Set when Player is in Canoe (modifies movement, allows movement in water, etc.)
    World_8_Dark:       .dsb 1   ; Darkness on World 8 Map, level 2 -- counts 0-7 while setting up the effect
    World_Map_AnimF:    .dsb 1   ; World map animation frame (for bushes, etc.)
    World_Map_AnimT:    .dsb 1   ; World map animation tick
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ...
    Event_Countdown:    .dsb 1

    Player_TailCount:   .dsb 1   ; Determines display frame of tail wag
    Player_InAir_OLD:   .dsb 1   ; Stores backup of Player_InAir
    Player_FireCount:       ; Player shoots fireball/hammer, sets sprite frame (shared with Player_FrogHopCnt)
    Player_FrogHopCnt:  .dsb 1   ; Counter used for frog hopping along the ground (shared with Player_FireCount)

                .dsb 1   ; $0514 unused

    Player_PMeterCnt:   .dsb 1   ; Tick counter used to count when to increase/decrease Power Meter
    B10Coin_Timer:      .dsb 1   ; Decrements until zero, which is how much time you have to get the max coins from a 10 coin block
    Player_TailAttack:  .dsb 1   ; Initiailized to $12; counts down to zero, performs tail attack!

    CineKing_Timer:         ; Timer; decrements to zero (shares Objects_Timer first byte)
    Objects_Timer:      .dsb 8   ; $0518-$051F "Timer" values; automatically decrements to zero
    ...
    Objects_Timer2:     .dsb 8   ; $0520-$0527 "Timer" values; automatically decrements to zero

    ; All the Level_BlockChgX/Y values are aligned to nearest 16 (i.e. tile coordinate)
    Level_BlockChgXHi:  .dsb 1   ; Player X High value when block change was queued
    Level_BlockChgXLo:  .dsb 1   ; Player X Low value when block change was queued
    Level_BlockChgYHi:  .dsb 1   ; Player Y High value when block change was queued
    Level_BlockChgYLo:  .dsb 1   ; Player Y Low value when block change was queued

    ; the block "bounce" that occurs after hitting ? block, music note block, etc.
    Level_BlkBump_Pos:  .dsb 2   ; $052C-$052D Block bump effect slot "position" (from 10 down, "position" of bounce)
    Level_BlkBump:      .dsb 3   ; $052E-$0530 Block bump effect slot (use Level_ChgTileEvent value, or 0 for inactive)
    Level_BlkBump_XHi:  .dsb 3   ; $0531-$0533 Block bump slot X Hi
    Level_BlkBump_XLo:  .dsb 3   ; $0534-$0536 Block bump slot X Lo
    Level_BlkBump_YHi:  .dsb 3   ; $0537-$0539 Block bump slot Y Hi
    Level_BlkBump_YLo:  .dsb 3   ; $053A-$053C Block bump slot Y Lo

                .dsb 5   ; $053D-$0541 unused

    ; The alternate vertical scrolls are used so that raster effects can be properly implemented!
    Level_VertScrollH:  .dsb 1   ; Alternate VertScroll_Hi used by engine, adjusted before being sent to Vert_Scroll_Hi
    Level_VertScroll:   .dsb 1   ; Alternate VertScroll used by engine, adjusted before being sent to Vert_Scroll

    Player_AboveTop:    .dsb 1   ; If Player is above top of level, this is $FF (-1), otherwise it is zero

    Level_InitAction:       ; AT LEVEL INITIALIZATION ONLY: Performs a specific initialization routine (NOTE: Shared with Player_Slide)
    Player_Slide:       .dsb 1   ; Positive values sliding forward, negative values sliding backward; directly sets Player_XVel

    Player_UphillFlag:  .dsb 1   ; When set, Player is walking uphill, and uses speed index value at Player_UphillSpeedIdx

                .dsb 8   ; $0547-$054E unused

    Player_Flip:        .dsb 1   ; Invincibility Somersault is used when set; only works in air, reset on ground

    Level_AirshipH:         ; Height of the airship during the Airship Intro (shared with Player_KuriboDir)
    Player_KuriboDir:   .dsb 1   ; While Kuribo's shoe is moving: 0 - Not requesting move, 1 - move right, 2 - move left

    Player_Grow:        .dsb 1   ; Tick counter used to animate growth into Super Mario (20 is starting value, or 2f to shrink)
    Player_FlashInv:    .dsb 1   ; Player "flashing invincibility" after being hit, counts down to zero
    Player_StarInv:     .dsb 1   ; Starman Invincibility counter; full/fatal invincibility, counts down to zero
    Player_SuitLost:    .dsb 1   ; Suit lost to hit discard "poof" counter
    Player_StarOff:     .dsb 1   ; Starman Invincibility wear-off (the last second or so when it slows and vanishes), counts down to zero
    Player_HaltTick:    .dsb 1   ; When non-zero, all action halts until this countdown hits zero
    Player_Kick:        .dsb 1   ; Kick frame in use, counts down to zero
    Player_PipeFace:    .dsb 1   ; Player forward frame, used for vertical pipe entrance, counts down to zero
    Player_EndLevel:    .dsb 1   ; Player's end of level run-off until count down to zero (player will actually wrap around horizontally if too large)
    Level_AirshipCtl:   .dsb 1   ; Airship control -- 1 = run and jump to catch air ship, 2 = climb to enter, 3 = enter

                .dsb 1   ; $055B unused

    Counter_Wiggly:     .dsb 1   ; "Wiggly" counter, provides rippled movement (like the airship rising during its intro)
    Counter_7to0:       .dsb 1   ; Counter that runs from 7 to 0 continuously while level is in progress

                .dsb 1   ; $055E unused

    LevelPartialInit:   .dsb 1   ; When set, performs a partial reinitialization of level data (notably does not perform the Level InitAction unless it is airship related)
    Level_TilesetIdx:   .dsb 1   ; Holds Level_Tileset as an "index" value instead, relative to levels (i.e. Level_Tileset - 1)
    Level_ChangeReset:  .dsb 1   ; When set to zero, a mass reset is performed (used when changing "scenes" in a single level)
    Level_UnusedFlag:   .dsb 1   ; Unused; only set in a couple places, but never read back!
    Level_SlopeEn:      .dsb 1   ; If set, enables slope tiles (otherwise they're considered flat top-only solids)
...
CHNGTILE_GIANTBRICKFIX  = $18   ; Giant World brick restore (small Mario hit giant brick)
    Level_ChgTileEvent: .dsb 1   ; When non-zero, queues a "change tile" event

    Level_NoStopCnt:    .dsb 1   ; A counter which continuously increments unless something is "stopping" the action
    Level_Event:        .dsb 1   ; Check "LevelEvent_Do" for values; 0 means nothing
    Level_PSwitchCnt:   .dsb 1   ; When non-zero, P-Switch is active (init @ $80); counts down to zero and restarts music

                .dsb 1   ; $0568 unused

    Player_SlideRate:   .dsb 1   ; While Player is sliding, this is added to X Velocity (does not persist, however)

                .dsb 1   ; $056A unused

    Player_IsClimbing:  .dsb 1   ; Set when Player is climing vine
    Player_FlipBits_OLD:    .dsb 1   ; Holds backup of Player_FlipBits
    Player_HitCeiling:  .dsb 1   ; Flag set when Player has just hit head off ceiling
    Player_FlyTime:     .dsb 1   ; When > 0, Player can fly (for power ups that do so); decrements (unless $FF) to 0
    Player_IsDucking:   .dsb 1   ; Set when Player is ducking down
    Player_WhiteBlkCnt: .dsb 1   ; White block counter; counts up while Player is standing on white block and holding down
    ...
    Level_PipeMove:     .dsb 1

    Level_CoinHeav:     .dsb 1   ; Enter coin heaven when set $80; Increments; at $D0, "soft jump" arrival; terminates at wrap to $00

    Player_MoveLR:      .dsb 1   ; 0 - Not moving left/right, 1 - Moving left, 2 - Moving right (reversed from the pad input)

    Player_WalkAnimTicks:   .dsb 1   ; Ticks between animation frames of walking; max value varies by Player's X velocity

    Player_InWater:     .dsb 1   ; Set for when in water (1 = Regular water specifically, other non-zero values indicate waterfall)
    Player_SwimCnt:     .dsb 1   ; Swim counter FIXME Describe better 0-3
    Player_Kuribo:      .dsb 1   ; Set for Kuribo's Shoe active
    Player_QueueSuit:   .dsb 1   ; Queues a suit change (values like Player_Suit, but add 1, EXCEPT: $0F = Statue enable, $40 = Splash, $80 = Kuribo's Shoe)
    Player_mGoomba:     .dsb 1   ; Player is caught by a micro Goomba (jump short)
    Player_Statue:      .dsb 1   ; Player is in Tanooki Statue mode; counts down to zero
    Player_RunFlag:     .dsb 1   ; Set while Player is actually considered "running" (holding down B and at enough speed; doesn't persist)
    Player_Bounce:      .dsb 1   ; Set to cause block bounce (upper 4 bits specifies what kind of block will be bounced)
    Player_BounceDir:   .dsb 1   ; Direction of Player bounce -- 0 = Down, 1 = Up, 2 = Left, 3 = Right
    Player_BounceObj:   .dsb 1   ; Set if it was a kicked shelled object that hit the bounce block (i.e. don't bounce the Player if the object is the one that hit)
    Counter_ByPlayerSpd:    .dsb 1   ; A counter which increments faster as the Player goes faster

    Level_HAutoScroll:  .dsb 1   ; When set to 1, "auto horizontal scroll" is active (this can be toggled mid-level)
    B10Coin_Count:      .dsb 1   ; Decrements until -1, you continue to get a coin until it does so
    B10Coin_ID:     .dsb 1   ; Forms a sort of unique ID so game knows if you've switched blocks

    Player_OffScreen:   .dsb 1   ; Set when Player is completely off screen
    ...
    FloatLevel_PlayerWaterStat: .dsb 1


    Player_LowClearance:    .dsb 1   ; Set when Player is in a "low clearance" situation (big Mario in a single block high tunnel)

    PUp_StarManFlash:   .dsb 1   ; Set when a Starman is bouncing about so it cycles colors! (also used to get super suit if set in advance)
    ...
    Player_Behind_En:   .dsb 1
    Player_Behind:      .dsb 1   ; When non-zero, Player is "behind the scenes" (as by white block)

    Player_Slippery:    .dsb 1   ; 0 = Ground is not slippery, 1 = Ground is a little slippery, 2 = Ground is REALLY slippery
    Player_SandSink:    .dsb 1   ; Sinking in quicksand! (holds Y when quicksand hit in upper 4 bits, bit 0 sets '0' jumping, '1' sinking)
    ...
    Player_PartDetEn:   .dsb 1
    Player_InPipe:      .dsb 1   ; Player is going through pipe
    Player_MushFall:    .dsb 1   ; Set to 0 when mushroom is to fall to the left, 1 to the right
    Player_SprOff:      .dsb 1   ; Player sprite offset (NOTE: Should be multiples of 4, otherwise bad unaligned stuff happens!)

    ; Strange gapping here; there's pretty much enough room for a couple more special objects
    Object_SprRAM:      .dsb 8   ; $058F-$0596 Sprite_RAM offset by object

    SpecialObj_Var2:    .dsb 8   ; $0597-$059E General purpose variable 2

                .dsb 2   ; $059F-$05A0 unused

    SpecialObj_YVelFrac:    .dsb 8   ; $05A1-$05A8 Y velocity fractional accumulator

                .dsb 2   ; $05A9-$05AA unused

    SpecialObj_XVelFrac:    .dsb 8   ; $05AB-$05B2 X velocity fractional accumulator

                .dsb 2   ; $05B3-$05B4 unused

    SpecialObj_Var3:    .dsb 8   ; $05B5-$05BC General purpose variable 3

                .dsb 2   ; $05BD-$05BE unused

    SpecialObj_YLo:     .dsb 8   ; $05BF-$05C6 Y low coordinate of special object

                .dsb 2   ; $05C7-$05C8 unused

    SpecialObj_XLo:     .dsb 8   ; $05C9-$05D0 X low coordinate of special object

                .dsb 2   ; $05D1-$05D2 unused

    SpecialObj_YVel:    .dsb 8   ; $05D3-$05DA Y Velocity of special object

                .dsb 2   ; $05DB-$05DC unused

    SpecialObj_XVel:    .dsb 8   ; $05DD-$05E4 X Velocity of special object

                .dsb 8   ; $05E5-$05EC unused

    Misc_Counter:       .dsb 1   ; Miscellaneous loop counter ?
    Level_TimerMSD:     .dsb 1   ; Leftmost / most significant digit on timer
    Level_TimerMid:     .dsb 1   ; Middle digit on timer
    Level_TimerLSD:     .dsb 1   ; Rightmost / least significant digit on timer
    Level_TimerTick:    .dsb 1   ; Timer decrementing tick
    Inventory_Open:     .dsb 1   ; Set when inventory panel is open, also used to dictate whether it is "opening" (1) or "closing" (0)
    Level_TimerEn:      .dsb 1   ; Set to disable clock (bit 7 will also disable level animations, e.g. '?s')
    Kill_Tally:     .dsb 1   ; Counter that increases with each successful hit of an object without touching the ground

    Objects_KillTally:  .dsb 5   ; $05F5-$05F9 OBJECT SLOTS 0 - 4 ONLY: Kill_Tally for a kicked shell as it hits other enemies

    PlayerProj_YHi:     .dsb 2   ; $05FA-$05FB Player projectile Y Hi
...
ASCONFIG_HDISABLE   = $80   ; Disables horizontal auto scroll coordinate adjustment (generally if Horz Auto Scroll is not in use)
    Level_AScrlConfig:  .dsb 1

    Cine_ToadKing:      .dsb 1   ; Set to 1, initializes Toad and transformed king; set to 2 while running that cinematic
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ...
    Level_Tile_Head:    .dsb 1   ; Tile at Player's head
    Level_Tile_GndL:    .dsb 1   ; Tile at Player's feet left
    Level_Tile_GndR:    .dsb 1   ; Tile at Player's feet right
    Level_Tile_InFL:    .dsb 1   ; Tile "in front" of Player ("lower", at feet)
    Level_Tile_InFU:    .dsb 1   ; Tile "in front" of Player ("upper", at face)
    Level_Tile_Whack:   .dsb 1   ; Tile last hit by tail attack or shell
    Level_Tile_Quad:    .dsb 4   ; $0608-$060B Quadrant of tile for each of the positions above
    ...
    Level_Tile_Slope:   .dsb 4   ; $060D-$0610

                .dsb 1   ; $0611 unused

    Scroll_Cols2Upd:    .dsb 1   ; Number of 8x8 columns to update (typically set to 32 for a full dirty update)

                .dsb 6   ; $0613-$0618 unused

    Bonus_CoinsYVel:    .dsb 6   ; $0619-$061E UNUSED Bonus game coins Y velocity
    Bonus_CoinsY:       .dsb 6   ; $061F-$0624 UNUSED Bonus game coins Y
    Bonus_CoinsXVel:    .dsb 6   ; $0625-$062A UNUSED Bonus game coins X velocity
    Bonus_CoinsX:       .dsb 6   ; $062B-$0630 UNUSED Bonus game coins X
    Bonus_CoinsYVelFrac:    .dsb 6   ; $0631-$0636 UNUSED Bonus game coins Y velocity fractional accumulator

                .dsb 6   ; $0637-$063C unused

    Bonus_CoinsXVelFrac:    .dsb 6   ; $063D-$0642 UNUSED Bonus game coins X velocity fractional accumulator

                .dsb 8   ; $0643-$064A unused

    Object_TileFeet:    .dsb 1   ; Object tile detected at "feet" of object
    Object_TileWall:    .dsb 1   ; Object tile detected in front of object, i.e. a wall

                .dsb 1   ; $064D unused

    Object_AttrFeet:    .dsb 1   ; Object tile quadrant of Object_TileFeet
    Object_AttrWall:    .dsb 1   ; Object tile quadrant of Object_TileWall

                .dsb 1   ; $0650 unused

    Objects_SprHVis:    .dsb 8   ; $0651-$0658 Flags; Bits 7-2 set when each 8x16 sprite is horizontally off-screen (left-to-right from MSb)
    Objects_SpawnIdx:   .dsb 8   ; $0659-$0660 Holds the index into level data that this object was spawned from
...
OBJSTATE_POOFDEATH  = 8 ; "Poof" Death (e.g. Piranha death)
    ...
    Objects_Frame:      .dsb 8   ; $0669-$0670 "Frame" of object (see ObjectGroup_PatternSets)

    Level_ObjectID:     .dsb 8   ; $0671-$0678 All active actor IDs

    Objects_FlipBits:   .dsb 8   ; $0679-$0680 Applied sprite attributes for this object (usually just horizontal/vertical flip)

    Objects_SprVVis:    .dsb 8   ; $0681-$0688 Flags; Bits 3-0 set when each 8x16 sprite is vertically off-screen (top-to-bottom from MSb)
    Objects_Var1:       .dsb 8   ; $0689-$0690 Generic variable 1 for objects
    Objects_Var2:       .dsb 8   ; $0691-$0698 Generic variable 2 for objects

    Unused699:      .dsb 1   ; Absolutely no idea, it is set once in one place and never used again... MAY be lost bonus game related?
    ...
    Score_Earned:       .dsb 2   ; $069C-$069D (16-bit value) A "buffer" of score earned to be added to your total, total score stored in Player_Score
    Score_Temp:     .dsb 1   ; Temp used when figuring out to display a 3-byte integer worth of score

                .dsb 5   ; $069F-$06A3 unused

    Player_IsHolding:   .dsb 1   ; Set when Player is holding something (animation effect only)
    Player_ISHolding_OLD:   .dsb 1   ; Holds onto whether Player WAS holding onto something (so we can be sure to clear Player_IsHolding)
...
;   In all other states, timer decrements every 2 ticks
    Objects_Timer3:     .dsb 5   ; $06A6-$06AA Used as the "wake up" out of shell timer
    Objects_Timer4:     .dsb 5   ; $06AB-$06AF "Timer" values; automatically decrements to zero (used in "shakin' awake" effect)

                .dsb 3   ; $06B0-$06B2 unused

    Object_SlopeHeight: .dsb 1   ; Object calculated slope height
    Buffer_Occupied:    .dsb 2   ; $06B4-$06B5 Set if respective Object_BufferX/Y buffer is already taken by an object

    Player_UphillSpeedIdx:      ; Override when Player_UphillFlag is set (shared with Player_Microgoomba)
    Player_Microgoomba: .dsb 1   ; Microgoomba stuck to Player
    Objects_InWater:    .dsb 5   ; $06B7-$06BB Set when object is in water

                .dsb 1   ; $06BC unused

    SpecialObj_Var1:    .dsb 8   ; $06BD-$06C4 General purpose variable 1

                .dsb 2   ; $06C5-$06C6 unused

    SpecialObj_Data:    .dsb 8   ; $06C7-$06CE Special object "data" field, defined by object

                .dsb 2   ; $06CF-$06D0 unused

    SpecialObj_Timer:   .dsb 8   ; $06D1-$06D8 "Timer" values; automatically decrements to zero

                .dsb 2   ; $06D9-$06DA unused

    CannonFire_Var:     .dsb 8   ; $06DB-$06E2
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ...
    LevLoad_Unused1:    .dsb 1
    LevLoad_Unused2:    .dsb 1
    LevLoad_Unused3:    .dsb 1
    LevLoad_Unused4:    .dsb 1

                .dsb 1   ; $0705 unused

    LL_ShapeDef:        .dsb 1   ; During level loading, defines a shape of something (context-specific)

    Scroll_UpdAttrFlag: .dsb 1   ; Set when it is time to update attributes

                .dsb 2   ; $0708-$0709 unused
    ...
    ; 17 = N-Spade
    ; 18 = 2P vs
    Level_Tileset:      .dsb 1   ; Different tilesets which changes the detection and meaning in levels

    Bonus_UnusedVH:         ; VRAM High address ?? Seems to only be part of an unused routine
    ToadTalk_VH:        .dsb 1   ; Cinematic Toad & King / "Toad House" Toad VRAM Address High

    Bonus_UnusedVL:         ; VRAM Low address ?? Seems to only be part of an unused routine
    ToadTalk_VL:        .dsb 1   ; Cinematic Toad & King / "Toad House" Toad VRAM Address Low

    BonusText_CPos:
    ToadTalk_CPos:      .dsb 1   ; Cinematic Toad & King / "Toad House" Toad Character Position

    BonusText_CharPause:    .dsb 1   ; Counter that decrements to zero between letters
    Bonus_UnusedFlag:   .dsb 1   ; Doesn't do much besides block an unused subroutine

    Map_Pan_Count:      .dsb 1   ; Map is panning, counts to zero (Scroll_LastDir sets which direction we're panning)

    ; NOTE sharing
    CineKing_Timer2:        ; Timer; decrements to zero
    Bonus_Timer:            ; Decrements to zero
    Map_Intro_Tick:     .dsb 1   ; Counts down to zero while displaying the "World X" intro

                .dsb 1   ; $0712 unused

    Map_ReturnStatus:   .dsb 1   ; When 0, level panel is cleared; otherwise, Player is considered to have died (decrements life!)
    MaxPower_Tick:      .dsb 1   ; When Player has maximum "power" charge, this counts for the flashing [P]
    Player_Score:       .dsb 3   ; $0715 (H)-$0717 (L) treated as 3-byte integer, with least significant zero on display not part of this value
    ...
    PatTable_BankSel:   .dsb 6   ; $0719-$071E  Provides an array of 6 pages to set the entire Pattern Table [BG_Full_CHRROM_Switch]
    PAGE_C000:      .dsb 1   ; Page to set PRG ROM C000 (PRGROM_Change_Both)
    PAGE_A000:      .dsb 1   ; Page to set PRG ROM A000 (PRGROM_Change_Both)
    PAGE_CMD:       .dsb 1   ; When using PRGROM_Change_Both2 or PRGROM_Change_A000, this value stores the MMC3 command
    Map_Prev_XOff:      .dsb 2   ; $0722-$0723 (Mario/Luigi) Stores previous scroll X offset on map
    Map_Prev_XHi:       .dsb 2   ; $0724-$0725 (Mario/Luigi) Stores previous "hi byte" of map X
    Player_Current:     .dsb 1   ; Which Player is currently up (0 = Mario, 1 = Luigi)
    World_Num:      .dsb 1   ; Current world index (0-8, where 0 = World 1, 7 = World 8, 8 = World 9 / Warp Zone)
    ...
    GameOver_State:     .dsb 1   ; State variable used during "Gameover!" sequence only; overlaps World_EnterState

    Map_Operation:      .dsb 1   ; Map_Operation: Current "operation" happening on map (See Map_DoOperation in PRG010)

                .dsb 1   ; $072A unused

    Total_Players:      .dsb 1   ; Total players (0 = 1P, 1 = 2P)
    Map_Unused72C:      .dsb 1   ; No apparent use; only assigned to, but never read back
    Bonus_DDDD:     .dsb 1   ; ?? Set to '1' if you exit the unused bonus game BONUS_UNUSED_DDDD
    Map_HandState:      .dsb 1   ; Hand trap state

                .dsb 1   ; $072F unused

    Map_WW_Backup_Y:    .dsb 1   ; Warp Whistle wind backs up the Player's map Y

    Map_WW_Backup_XH:   .dsb 1   ; Warp Whistle wind backs up the Player's map X Hi byte
    Map_WW_Backup_X:    .dsb 1   ; Warp Whistle wind backs up the Player's map X
    Map_WW_Backup_UPV2: .dsb 1   ; Warp Whistle wind backs up Map_UnusedPlayerVal2

                .dsb 2   ; $0734-$0735 unused

    Player_Lives:       .dsb 2   ; $0736-$0737 (Mario/Luigi) Player's lives
    Map_Unused738:      .dsb 1   ; Index used in some dead code in PRG011; sometimes index of unused array Map_Unused7DC6
    ClearPattern:       .dsb 1   ; Set by ClearPattern_ByTileset for use in Clear_Nametable_Short
    PalSel_Tile_Colors: .dsb 1   ; Stores value to index which tile color set to use when palette loading routine is called
    PalSel_Obj_Colors:  .dsb 1   ; Stores value to index which object color set to use when palette loading routine is called
    Player_FallToKing:  .dsb 2   ; $073C-$073D (Mario/Luigi) Player will fall to king when Level_ExitToMap is nonzero (instead of map)
    Map_Player_SkidBack:    .dsb 2   ; $073E-$073F (Mario/Luigi) Set to '1' if Player died last turn or otherwise ejected (that is, they DID skid from their last turn, not necessarily currently skidding)

    Map_NSpadeMatches:  .dsb 1   ; Keeps count of N-Spade matches of active session (9 means game is done)
    Map_NSpadeInProg:   .dsb 1   ; If set, there's an N-Spade game "in progress" (so if you lose, you pick it up later)

    Map_Airship_Trav:   .dsb 1   ; Airship's current travel-table offset (randomly offset by 0-2, spices up life)

                .dsb 2   ; $0743-$0744 unused

    Map_DoFortressFX:   .dsb 1   ; Bust locks, build bridges, whatever after Mini-Fortress is toppled
    ...
    Object_VelCarry:    .dsb 1   ; '1' when last Object Velocity fraction accumulation rolled over

    ; WARNING: The distance between Player/Objects_XVelFrac and Player/Objects_YVelFrac must be same as Player/Objects_X/YVel!
    Player_XVelFrac:    .dsb 1   ; X velocity fractional accumulator
    Objects_XVelFrac:   .dsb 8   ; $074E-$0755 Other object's X velocity fractional accumulator

                .dsb 1   ; $0756 unused

    THouse_UnusedFlag:  .dsb 1   ; Cleared when Toad House gives you an item, but never used otherwise
                ...
                .dsb 7   ; $0758-$075E unused

    ; WARNING: The distance between Player/Objects_XVelFrac and Player/Objects_YVelFrac must be same as Player/Objects_X/YVel!
    Player_YVelFrac:    .dsb 1   ; Y velocity fractional accumulator
    Objects_YVelFrac:   .dsb 8   ; $0760-$0767 Other object's Y velocity fractional accumulator

    Objects_ColorCycle: .dsb 8   ; $0768-$076F Cycles colors of object and decrements to zero (e.g. "Melting" ice block, starman, etc.)
    ...
    Objects_Var6:       .dsb 5   ; $0770-$0774 General purpose variable 6 (except as noted above)
    Objects_TargetingXVal:  .dsb 5   ; $0775-$0779 X velocity result of Object_CalcHomingVels for this object OR some other X pixel target

    King_Y:             ; Y position (NOTE: shared with Objects_TargetingYVal)
    Objects_TargetingYVal:  .dsb 5   ; $077A-$077E Y velocity result of Object_CalcHomingVels for this object OR some other Y pixel target

    Pipe_TransYDelta:       ; In-level transit pipe Y delta value (WARNING: Shared with Level_ScrollDiffV)
    Level_ScrollDiffV:  .dsb 1   ; Difference between desired vertical and the current Vert_Scroll (WARNING: Shared with Pipe_TransYDelta)
    Level_ScrollDiffH:  .dsb 1   ; Difference between desired horizontal and the current Horz_Scroll

    Random_Pool:        .dsb 10  ; $0781-$078A (or $0789?) Data pool for pseudo-random number generator algorithm
RandomN = Random_Pool+1         ; Pull a random number from the sequence (NOTE: RandomN+1 is also good; If you need multiple random numbers, call Randomize)
    ...
    Map_PlayerLost2PVs: .dsb 1   ; When > 0, (1=Mario, 2=Luigi) doesn't lose a life for "death" exiting to map, but does lose their turn
    ...
    Objects_PlayerHitStat:  .dsb 8   ; $0796-$079D Player hit status

    ; Up to five "scores" can be displayed at once
    Scores_Value:       .dsb 5   ; $079E-$07A2 score "value"; '0' none, (10, 20, 40, 80, 100, 200, 400, 800, 1000, 2000, 4000, 8000, 1-up)
    Scores_Counter:     .dsb 5   ; $07A3-$07A7 "counter" until score disappears
    Scores_Y:       .dsb 5   ; $07A8-$07AC Score's Y
    Scores_X:       .dsb 5   ; $07AD-$07B1 Score's X

    LRBounce_Y:     .dsb 1   ; Left/right bouncer as sprite Y ($FF is disabled)
    LRBounce_X:     .dsb 1   ; Left/right bouncer as sprite X
    LRBounce_Vel:       .dsb 1   ; Left/right bouncer absolute value of X velocity

    ; NOTE!! These object vars are OBJECT SLOT 0 - 4 ONLY!
    Objects_Slope:      .dsb 5   ; $07B5-$07B9 Absolute slope calc value

                .dsb 1   ; $07BA unused

    World3_Bridge:      .dsb 1   ; 0 - Bridges are down, 1 - Bridges are up

    ArrowPlat_IsActive: .dsb 1   ; Set if arrow platform is active

    Level_GetWandState: .dsb 1   ; See Koopaling code in PRG001
...
; style of the Video_Upd_Table; see "Video_Upd_Table" in PRG030 for format.
    ...
    Palette_AddrHi:     .dsb 1   ; Stores high part of palette address when committing palettes
    Palette_AddrLo:     .dsb 1   ; Stores low part of palette address when committing palettes
    Palette_BufCnt:     .dsb 1   ; 32 for updating entire palette
    Palette_Buffer:     .dsb 32  ; $07C1-$07E0 Buffer of palette bytes to commit, used for fade in/out
    Palette_Term:       .dsb 1   ; Set to zero as terminator, per requirement of the Video_Upd_Table format
; ********************************************************************************
...
;   are not re-enterable, but still seems a bit extreme...
    BigQBlock_GotIt:    .dsb 1

                .dsb 13  ; $07E3-$07EF unused

    DMC_Queue:      .dsb 1   ; Stores value to play on DMC
    DMC_Current:        .dsb 1   ; Currently playing DMC sound

    Sound_Sq1_CurFL:    .dsb 1   ; Holds current "low" frequency of Square Wave 1 (Warning: Must be +4 to Sound_Sq2_CurFL, see PRG031_E808)

    Music_NseStart:     .dsb 1   ; Holds the starting offset of the noise track (CHECK: Reuse of $07F3, is this bad??)

                .dsb 1   ; $07F4 unused, but required for padding

    Music2_Hold:        .dsb 1   ; A very little used feature, Music Set 1 overrides Music Set 2, but after a M1 song finishes, it restarts the M2 song
    Sound_Sq2_CurFL:    .dsb 1   ; Holds current "low" frequency of Square Wave 1 (Warning: Must be +4 from Sound_Sq1_CurFL, see PRG031_E808)

    Music_Sq2Patch:     .dsb 1   ; Current "instrument patch" for Square 2 (only upper 4 bits stored, 0ppp 0000)
    Music_Sq1Patch:     .dsb 1   ; Current "instrument patch" for Square 1 (only upper 4 bits stored, 0ppp 0000)

                .dsb 1   ; $07F9 unused

    Sound_Map_L2Hld:    .dsb 1   ; Same as Sound_Map_LHold, used for the secondary track (Warning: Will be affected by triangle, see PRG031_E808)
    Sound_Map_Len2:     .dsb 1   ; Same as Sound_Map_Len, used for the secondary track
    Sound_Map_LHold:    .dsb 1   ; Current length setting, used as delay after each byte of map sound (changed by special bytes in range $80-$FF)
    Sound_Map_Len:      .dsb 1   ; Countdown tick for current note/rest that map sound effect is on
...
; 2P Vs Only
    ...
    ; 6: Dying
    ; 7: Climbing
    Vs_PlayerFrame:     .dsb 2   ; $6800-$6801 Mario/Luigi Frame
    Vs_PlayerState:     .dsb 2   ; $6802-$6803 Mario/Luigi State (0=Init, 1=Normal, 2=Dying, 3=Ladder climbing)
    Vs_ObjectState:     .dsb 12  ; $6804-$680F Objects State (0=Dead/empty, 1=Normal, 2=Flipped over, 3=Dying)

                .dsb 1   ; $6810 unused

    Vs_PlayerBlkHitCnt: .dsb 2   ; $6811-$6812 Mario/Luigi Hit block counter value
    Vs_PlayerY:     .dsb 2   ; $6813-$6814 Mario/Luigi Y
    Vs_ObjectsY:        .dsb 12  ; $6815-$6820 Objects Y

                .dsb 1   ; $6821 unused

    Vs_PlayerBlkHitY:   .dsb 2   ; $6822-$6823 Mario/Luigi Aligned Y position where block was hit
    Vs_PlayerX:     .dsb 2   ; $6824-$6825 Mario/Luigi X
    Vs_ObjectsX:        .dsb 12  ; $6826-$6831 Objects X

                .dsb 1   ; $6832 unused

    Vs_PlayerBlkHitX:   .dsb 2   ; $6833-$6834 Mario/Luigi Aligned Y position where block was hit
    Vs_PlayerYVel:      .dsb 2   ; $6835-$6836 Mario/Luigi Y Velocity
    Vs_ObjectYVel:      .dsb 12  ; $6837-$6842 Objects Y Velocity

                .dsb 1   ; $6843 unused

    Vs_PlayerBlkHitYVel:    .dsb 2   ; $6844-$6845 Mario/Luigi Hit block Y velocity
    Vs_PlayerXVel:      .dsb 2   ; $6846-$6847 Mario/Luigi X Velocity
    Vs_ObjectXVel:      .dsb 12  ; $6848-$6853 Objects X Velocity

                .dsb 3   ; $6854-$6856 unused

    Vs_PlayerClimbFrame:    .dsb 2   ; $6857-$6858 incremented as Player climbs
    Vs_ObjectAnimCnt:   .dsb 12  ; $6859-$6864 A continuous counter per object for animating (typically 2 frames)

                .dsb 1   ; $6865 unused

    Vs_PlayerDir:       .dsb 2   ; $6866-$6867 Mario/Luigi direction (1=Right, 2=Left)
    Vs_ObjectDir:       .dsb 12  ; $6868-$6873 Objects direction (1=Right, 2=Left)

                .dsb 1   ; $6874 unused

    Vs_PlayerYVelFrac:  .dsb 2   ; $6875-$6876 Mario/Luigi Y velocity fractional accumulator
    Vs_ObjectYVelFrac:  .dsb 12  ; $6877-$6882

                .dsb 3   ; $6883-$6885 unused

    Vs_PlayerXVelFrac:  .dsb 2   ; $6886-$6887 Mario/Luigi X velocity fractional accumulator
    Vs_ObjectXVelFrac:  .dsb 12  ; $6888-$6893

                .dsb 3   ; $6894-$6896 unused

    Vs_PlayerDetStat:   .dsb 2   ; $6897-$6898 Mario/Luigi detection status
    Vs_ObjectDetStat:   .dsb 12  ; $6899-$68A4 Objects detection status

                .dsb 3   ; $68A5-$68A7 unused

    Vs_ObjectVar1:      .dsb 12  ; $68A8-$68B3 General variable 1

                .dsb 1   ; $68B4 unused

    Vs_PlayerKick:      .dsb 2   ; $68B5-$68B6 Mario/Luigi Player is kicking until decrements to zero
    Vs_PlayerDizzy:     .dsb 2   ; $68B7-$68B8 Mario/Luigi Player "dizzy" face until decrements to zero
    Vs_PlayerStick:     .dsb 2   ; $68B9-$68BA Mario/Luigi Mario/Luigi Player "sticking" to ceiling; decrements to zero
    Vs_PlayerBumpTimer: .dsb 1   ; Mario/Luigi Players bumped off eachother (and can't again until zero); decrements to zero
    Vs_POWBlockCnt:     .dsb 1   ; POW block counter; decrements to zero; until then, POW shaking!
...
VSOBJID_KICKEDBLOCK = 11    ; Kicked block (from [?] block match)
    Vs_ObjectId:        .dsb 12  ; $68BD-$68C8 Objects ID

                .dsb 1   ; $68C9 unused

    Vs_ObjectSprRAMOff: .dsb 1   ; Current object Sprite RAM offset
    Vs_ObjectSprRAMSel: .dsb 1   ; Counter that runs $D to $0 (inclusive) and helps distribute Sprite RAM offsets among the objects
    Vs_EnemyCount:      .dsb 1   ; Number of spawned enemies (in the typical game)
    Vs_PlayerHaltTimer: .dsb 2   ; $68CD-$68CE Mario/Luigi timer which halts gameplay; decrements to zero
    Vs_ObjHaltTimer:    .dsb 12  ; $68CF-$68DA Object timer which halts object when greater than zero; decrements to zero

                .dsb 1   ; $68DB unused

    Vs_ObjectTimer3:    .dsb 12  ; $68DC-$68E7

                .dsb 1   ; $68E8 unused

    Vs_PlayerCnt:       .dsb 2   ; $68E9-$68EA Mario/Luigi "counter" value; decrements to zero
    Vs_EnemyGetUpTimer: .dsb 12  ; $68EB-$68F6 Timer for flipped-over enemy; decrements to zero

                .dsb 1   ; $68F7 unused

    Vs_PlayerJumped:    .dsb 2   ; $68F8-$68F9 Set to 1 if Player jumped; prevents Player from jumping again until they hit floor
    Vs_PlayerTileL:     .dsb 2   ; $68FA-$68FB Mario/Luigi Tile detected at Player's feet
    Vs_ObjectTileL:     .dsb 12  ; $68FC-$6907

                .dsb 2   ; $6908-$6909 unused

    Vs_PlayerBlkHit:    .dsb 2   ; $690A-$690B Mario/Luigi Holds Tile_Mem offset to bounce block they hit
    Vs_PlayerFlashInv:  .dsb 2   ; $690C-$690D Mario/Luigi Flashing invicibility (?)
    Vs_SpawnCnt2:       .dsb 1   ; FIXME describe better
    Vs_TooLongCnt:      .dsb 1   ; Increments after each round of spawning; if it overflows, "game ender" fireballs are spawned
    Vs_CurIndex:        .dsb 1   ; Current index (Player or object)
    Vs_PlayerTileU:     .dsb 2   ; $6911-$6912 Mario/Luigi Tile detected above Player's feet

                .dsb 13  ; $6913-$691F unused

    Vs_ObjectPipeTimer: .dsb 12  ; $6920-$692B Timer used for enemies to exit and emerge from pipes; decrements to zero

                .dsb 1   ; $692C unused

    Vs_Random:      .dsb 3   ; $692D-$692F Random generator for 2P Vs mode
    Vs_PlayerCoins:     .dsb 2   ; $6930-$6931 Player's coins (in 2P Vs); 5 wins the match
    Vs_TimeToExit:      .dsb 1   ; Decrements to zero then exits the 2P Vs
    Vs_ObjectIsLast:    .dsb 12  ; $6933-$693E Set if this is the last object (turns blue, move fast)

                .dsb 1   ; $693F unused

    Vs_POWHits:     .dsb 1   ; Number of times POW block has been hit (disabled on 3)
    Vs_PlayerYOff:      .dsb 2   ; $6941-$6942 Mario/Luigi Y offset applied
    Vs_UNKGAMECnt:      .dsb 1   ; Unknown "game" counter; after overflow, we exit
    Vs_PlayerYHi:       .dsb 2   ; $6944-$6945 Mario/Luigi Y Hi
    Vs_ObjectYHi:       .dsb 12  ; $6946-$6951 Object Y Hi

                .dsb 3   ; $6952-$6954 unused
    Vs_ObjectIsAngry:   .dsb 12  ; $6955-$6960 Set when Sidestepper is angry (not used for anything else)

                .dsb 1   ; $6961 unused

    Vs_AngrySidesteppers:   .dsb 1   ; When greater than zero, and spawning a Sidestepper, next one is an "angry" Sidestepper (then decrement)

                .dsb 1   ; $6963 unused

    Vs_ObjectVDir:      .dsb 12  ; $6964-$696F Objects vertical direction (4=Down, 8=Up)

                .dsb 1   ; $6970

    Vs_ObjectRestoreXVel:   .dsb 12  ; $6971-$697C Flipped over object restore X velocity

                .dsb 1   ; $697D unused

    Vs_ObjTimer2:       .dsb 12  ; $697E-$6989 Object timer; decrements to zero

                .dsb 1   ; $698A unused

    Vs_CardFlash:       .dsb 2   ; $698B-$698C Mario/Luigi Cycles color for card (when picked up from another Player)
    Vs_HaltTimerBackup: .dsb 15  ; $698D-$699B Backs up all halt timers
    Vs_EnemySet:        .dsb 1   ; Specifies an index of active enemy set, selecting one of the quintuples from Vs_5EnemySets
    Vs_ObjectXOff:      .dsb 1   ; A one-shot X offset for display of object FIXME: When?
    Vs_PlayerWalkCnt:   .dsb 2   ; $699E-$699F Mario/Luigi counts up and overflows to toggle walk frames
    Vs_PlayerWalkFrame: .dsb 2   ; $69A0-$69A1 Mario/Luigi incremented when Vs_PlayerWalkCnt overflows
    Vs_NextObjectIsLast:    .dsb 1   ; If there are 5 enemies and this is set, next enemy out is the "last" (turns blue, moves fast)

    ; Display of "x Up" after getting 3 cards
    Vs_xUpCnt:      .dsb 2   ; $69A3-$69A4 Mario/Luigi "x Up" counter
    Vs_xUpY:        .dsb 2   ; $69A5-$69A6 Mario/Luigi "x Up" Y pos
    Vs_xUpX:        .dsb 2   ; $69A7-$69A8 Mario/Luigi "x Up" X pos
...
Tile_Mem:   .dsb 6480    ; $6000-$794F Space used to store the 16x16 "tiles" that make up the World Map or Level

    Map_MoveRepeat:     .dsb 2   ; $7950-$7951 (Mario/Luigi) counts up to $18 and then you keep moving without pause
    AScrlURDiag_OffsetX:    .dsb 1   ; When diagonal autoscroller is wrapping, this holds an X offset for Player/Objects to temporarily correct
    AScrlURDiag_OffsetY:    .dsb 1   ; When diagonal autoscroller is wrapping, this holds an Y offset for Player/Objects to temporarily correct
    StatusBar_UpdFl:    .dsb 1   ; Status bar Update Flag; toggles so to update status bar only every other frame
    UpdSel_Disable:     .dsb 1   ; When set, disables the Update_Select routine during the NMI, which halts most activity due to no reported V-Blanking
    Map_Objects_Itm:    .dsb 13  ; $7956-$795D, "Item given by" map objects
    ...
    ; C = Warp Whistle
    ; D = Music Box
    Level_TreasureItem: .dsb 1
    Reset_Latch:        .dsb 1   ; If this value is anything besides $5A, the reset is run (I assume this is considered a safe value in case of data corruption, e.g. from runaway generator)
    Map_BonusType:      .dsb 1   ; 0 = No bonus, 1 = White Toad House, 2 = UNKNOWN WHITE THING (MAPOBJ_UNK0C)
    Map_BonusCoinsReqd: .dsb 1   ; Number of coins you need for White Toad House (or the MAPOBJ_UNK0C thing!); value ranges 0-127
    Coins_ThisLevel:    .dsb 1   ; Internal counter of coins earned -this level- (so always starts at 0 and increments)

    Map_NSpade_NextScore:   .dsb 3   ; $7968 (H)-$796A (L) treated as 3-byte integer

    Map_BonusAppY:      .dsb 1   ; Map "white" bonus appearance Y (set to Player's last "succeeded" map position)
    Map_BonusAppXHi:    .dsb 1   ; Map "white" bonus appearance XHi (set to Player's last "succeeded" map position)
    Map_BonusAppX:      .dsb 1   ; Map "white" bonus appearance X (set to Player's last "succeeded" map position)

    Map_NoLoseTurn:     .dsb 1   ; If set, Player does not lose turn after having completed a level (used for Toad House, pipeways, etc.)
    Map_Got13Warp:      .dsb 1   ; Set non-zero if Player already got the 1-3 Warp Whistle
    Map_Anchored:       .dsb 1   ; Set if anchor is set on this map
    Map_WhiteHouse:     .dsb 1   ; Set if you have already earned the White Toad House for this world
    Map_CoinShip:       .dsb 1   ; Set if you have already earned the Coin Ship for this world
    Map_WasInPipeway:   .dsb 1   ; Set if you just came out of a pipeway
    EndCard_Flag:       .dsb 1   ; Set when End Level card is hit (can determine when level has ended)
    Map_PlyrSprOvrY:    .dsb 1   ; "Player Sprite Override Y"; If set to $F8 during warp, erases Player's map sprite; otherwise provides a Y to put it at
    Map_Entered_Y:      .dsb 2   ; $7976-$7977 (Mario/Luigi) Stores the Y value when you enter a level; this is the Y used if you complete the level
    Map_Entered_XHi:    .dsb 2   ; $7978-$7979 (Mario/Luigi) Hi byte for Map_Entered_X
    Map_Entered_X:      .dsb 2   ; $797A-$797B (Mario/Luigi) Same as Map_Entered_Y, only for X
    Map_Previous_UnusedPVal2:.dsb 2  ; $797C-$797D (Mario/Luigi) Backup of Map_UnusedPlayerVal2
    Map_Previous_Y:     .dsb 2   ; $797E-$797F (Mario/Luigi) Stores the previous Y you were "safe" at; this is the Y you go back to if you die
    Map_Previous_XHi:   .dsb 2   ; $7980-$7981 (Mario/Luigi) Same as Map_Previous_Y, only for XHi
    Map_Previous_X:     .dsb 2   ; $7982-$7983 (Mario/Luigi) Same as Map_Previous_Y, only for X
    Map_Unused7984:     .dsb 2   ; $7984-$7985 (Mario/Luigi) Unused; cleared and never touched again
    Map_Prev_XOff2:     .dsb 2   ; $7986-$7987 (Mario/Luigi) Holds a copy of Map_Prev_XOff, but I'm not sure why?
    Map_Prev_XHi2:      .dsb 2   ; $7988-$7989 (Mario/Luigi) Holds a copy of Map_Prev_XHi, but I'm not sure why?
    Map_Unused798A:     .dsb 2   ; $798A-$798B (Mario/Luigi) Unused; cleared and never touched again

    ; These define values to use when you junction back
    ; to the level you were before...
    Level_Jct_HSHi:     .dsb 1   ; Level junction horizontal scroll high value
    Level_Jct_HS:       .dsb 1   ; Level junction horizontal scroll value
    Level_Jct_VSHi:     .dsb 1   ; Level junction vertical scroll high value
    Level_Jct_VS:       .dsb 1   ; Level junction vertical scroll value

                .dsb 2   ; $7990-7991 unused
    ...
    Bonus_DiePos:       .dsb 1   ; UNUSED Die in the lost bonus games, counts 0-5

    Map_Previous_Dir:   .dsb 2   ; $7993-$7994 (Mario/Luigi) Backup movement dir (remember which way Player moved last) (8=Up, 4=Down, 2=Left, 1=Right)

    Map_Unused7995:     .dsb 1   ; Unused; cleared but never used otherwise
...
CFIRE_LASER     = $15   ; Laser fire
    ...
    CannonFire_ID:      .dsb 8   ; $7A15-$7A1C ID of the cannon fire
    CannonFire_YHi:     .dsb 8   ; $7A1D-$7A24 Cannon fire Y Hi
    CannonFire_Y:       .dsb 8   ; $7A25-$7A2C Cannon fire Y
    CannonFire_XHi:     .dsb 8   ; $7A2D-$7A34 Cannon fire X Hi
    CannonFire_X:       .dsb 8   ; $7A35-$7A3C Cannon fire X
    CannonFire_Parent:  .dsb 8   ; $7A3D-$7A44 Tie back to level object index of "parent" object

    Splash_DisTimer:    .dsb 1   ; Player water splashes are disabled until decrements to zero; set when Player hits any bounce block

    ; For that little "flash" that comes from the shell kill impact!
    ShellKillFlash_Cnt: .dsb 1   ; "Shell Kill Flash" counter
    ShellKillFlash_Y:   .dsb 1   ; "Shell Kill Flash" Y
    ShellKillFlash_X:   .dsb 1   ; "Shell Kill Flash" X

; NOTE!! Objects_DisPatChng for OBJECT SLOT 0 - 5 ONLY!
    Objects_DisPatChng: .dsb 6   ; $7A49-$7A4E If set, this object no longer enforces a pattern bank change

; NOTE!! These object vars are OBJECT SLOT 0 - 5 ONLY!
    ObjSplash_DisTimer: .dsb 6   ; $7A4F-$7A54 Object water/lava splashes are disabled until decrements to zero

    PlayerProj_XVelFrac:    .dsb 2   ; $7A55-$7A56 Player Projectile X velocity fractional accumulator

    CannonFire_Timer2:  .dsb 8   ; $7A57-$7A5E Cannon Fire timer (decrements to zero)

    Roulette_Unused7A5F:    .dsb 1   ; Unused value in Roulette game
    Roulette_Unused7A5F_Delta:.dsb 1 ; Delta value added to Roulette_Unused7A5F

    Bowser_Tiles:       .dsb 2   ; $7A61-$7A62 Bowser's detected tiles (to determine what to break)
    Bowser_Counter1:    .dsb 1   ; A counter used by Bowser, decrements to zero
    Bowser_Counter2:    .dsb 1   ; A counter used by Bowser, decrements to zero
    Bowser_Counter3:    .dsb 1   ; A counter used by Bowser, random setting, decrements to zero

    CoinShip_CoinGlowIdx:   .dsb 1   ; Coin Ship only: Glowing coins palette color index
    CoinShip_CoinGlowCnt:   .dsb 1   ; Coin Ship only: Glowing coins palette color counter

    SObjBlooperKid_OutOfWater:.dsb 8 ; $7A68-$7A6F Blooper kid only; if set, Blooper Kid is trying to go out of water

                .dsb 2   ; $7A70-$7A71 unused

    Object_SplashAlt:   .dsb 1   ; Used to alternate the "splash slots" 1 and 2 as objects hit the water

                .dsb 109 ; $7A73-$7ADF unused

    Music_Start:        .dsb 1   ; Music start index (beginning of this song)
    Music_End:      .dsb 1   ; Music end index (inclusive last index to play before loop)
    Music_Loop:     .dsb 1   ; Music loop index (index to start from when song reaches end)

    Sound_Octave:       .dsb 1   ; Used for calculating octave

                .dsb 12  ; $7AE4-$7AEF unused
    ...
    Music_Sq2Bend:      .dsb 1   ; Alters PAPU_FT2 for bend effects
                ...
                .dsb 2   ; $7AF5-$7AF6 unused

    Music_RestH_Off:    .dsb 1   ; Offset added to Music_RestH_Base; typically $00 or $10 (for low time warning on compatible songs)

                .dsb 7   ; $7AF8-$7AFE unused

    PAPU_MODCTL_Copy:   .dsb 1   ; Current PAPU_MODCTL register

    Level_ObjIdxStartByScreen:.dsb 16    ; $7B00-$7B0F Defines the starting index into Level_Objects for each "screen"

    Level_ObjectsSpawned:   .dsb 48  ; $7B10-$7B3F When $80 set, object is already spawned, $00 means not
...
; The rest is a repeating series of 3 bytes -- ID, Column, Row (C/R of tile grid, multiply by 16 for pixel location), $FF for terminator
    Level_Objects:      .dsb 48*3    ; $7B40-$7BCF

                .dsb 80  ; $7BD0-$7C1F unused
...
; "Object_Delete" for the hardcoded list of objects which must release this resource...
    Object_BufferX:     .dsb 32*2    ; $7C20-$7C3F / $7C40-$7C5F
    Object_BufferY:     .dsb 32*2    ; $7C60-$7C7F / $7C80-$7C9F

; Variables used by Chain Chomps ONLY -- manages the chain links
    ChainChomp_ChainX1: .dsb 5   ; $7CA0-$7CA4 Chain Link 1 X
    ChainChomp_ChainX2: .dsb 5   ; $7CA5-$7CA9 Chain Link 2 X
    ChainChomp_ChainX3: .dsb 5   ; $7CAA-$7CAE Chain Link 3 X
    ChainChomp_ChainX4: .dsb 5   ; $7CAF-$7CB4 Chain Link 4 X

    ChainChomp_ChainY1: .dsb 5   ; $7CB4-$7CB8 Chain Link 1 Y
    ChainChomp_ChainY2: .dsb 5   ; $7CB9-$7CBD Chain Link 2 Y
    ChainChomp_ChainY3: .dsb 5   ; $7CBE-$7CC2 Chain Link 3 Y
    ChainChomp_ChainY4: .dsb 5   ; $7CC3-$7CC8 Chain Link 4 Y

; NOTE!! These object vars are OBJECT SLOT 0 - 4 ONLY!
    Objects_Var10:      .dsb 5   ; $7CC8-$7CCC Generic object variable 10
    Objects_Var11:      .dsb 5   ; $7CCD-$7CD1 Generic object variable 11
    Objects_Var12:      .dsb 5   ; $7CD2-$7CD6 Generic object variable 12
    Objects_Var13:      .dsb 5   ; $7CD7-$7CDB Generic object variable 13
    Objects_Var14:      .dsb 5   ; $7CDC-$7CE0 Generic object variable 14

; Player's hammer/fireball
    PlayerProj_ID:      .dsb 2   ; $7CE1-$7CE2 Player projectile ID (0 = not in use, 1 = fireball, 2 = hammer, 3+ = Fireball impact "Poof")
    PlayerProj_Y:       .dsb 2   ; $7CE3-$7CE4 Player projectile Y
    PlayerProj_X:       .dsb 2   ; $7CE5-$7CE6 Player projectile X
    PlayerProj_YVel:    .dsb 2   ; $7CE7-$7CE8 Player projectile Y Velocity (NOTE: Integer, not 4.4FP)
    PlayerProj_XVel:    .dsb 2   ; $7CE9-$7CEA Player projectile X Velocity (NOTE: Fireball is integer, 4.4FP for hammer ONLY)
    Fireball_HitChkPass:    .dsb 2   ; $7CEB-$7CEC Count of times Player's fireball has gone through hit check; when it hits 2, fireball poofs
    PlayerProj_Cnt:     .dsb 2   ; $7CED-$7CEE Player projectile counter

    Temp_VarNP0:        .dsb 1   ; A temporary not on page 0

    Lakitu_Active:      .dsb 1   ; Set while a Lakitu is active; keeps Lakitu "alive" even if off-screen etc.

    LevelEvent_Cnt:     .dsb 1   ; General purpose counter used by a couple LevelEvents
    Vert_Scroll_Off:    .dsb 1   ; Vertical scroll offset, used for "vibration" effects
    Level_Vibration:    .dsb 1   ; While greater than zero, screen vibrates (from impact of heavy fellow)
    Player_VibeDisable: .dsb 1   ; While greater than zero, Player is unable to move (from impact of heavy fellow)
    Player_TwisterSpin: .dsb 1   ; While greater than zero, Player is twirling from sand twister

; NOTE!! This object var is OBJECT SLOT 0 - 4 ONLY!
    Objects_HitCount:   .dsb 5   ; $7CF6-$7CFA Somewhat uncommon "HP" used generally for bosses only (e.g. they take so many fireballs)


    RotatingColor_Cnt:  .dsb 1   ; When non-zero, causes rainbow palettes in the background; $80 bit is used by Koopaling wand grab

; Some variables used by the recovered magic wand
    Wand_FrameCnt:      .dsb 1   ; A counter that overflows to increment Wand_Frame (added to by the wand's SpecialObj_Var1)
    Wand_Frame:     .dsb 1   ; Wand frame
    Wand_BounceFlag:    .dsb 1   ; Tracks the recovered wand bounce; odd on first bounce

    Player_DebugNoHitFlag:  .dsb 1   ; UNUSED: (Old debug routine) When set, disables getting hurt (would be toggled by pressing SELECT; see PRG000 $C91B)
...
; $01
; --------- BOT OF MAP
    Map_Completions:    .dsb 16*4*2  ; $7D00-$7D3F (Mario), $7D40-$7D7F (Luigi) Allows a MAX of 4 map screens (64 columns total each player, 16 columns per map screen)
    ...
    ; C = Warp Whistle
    ; D = Music Box
    Inventory_Items:    .dsb 4*7 ; $7D80-$7D9B Mario, 4 rows of 7 items
    Inventory_Cards:    .dsb 3   ; $7D9C-$7D9E Mario, 3 cards
    Inventory_Score:    .dsb 3   ; $7D9F-$7DA1 Mario, 3 byte score
    Inventory_Coins:    .dsb 1   ; Mario's coins

    Inventory_Items2:   .dsb 4*7 ; $7DA3-$7DBE Luigi, 4 rows of 7 items
    Inventory_Cards2:   .dsb 3   ; $7DBF-$7DC1 Luigi, 3 cards
    Inventory_Score2:   .dsb 3   ; $7DC2-$7DC4 Luigi, 3 byte score
    Inventory_Coins2:   .dsb 1   ; Luigi's coins
    Map_Unused7DC6:     .dsb 5   ; $7DC6-$7DCA? Indexed by Map_Unused738, value used in dead routine in PRG011 @ $A2AF

    Map_GameOver_CursorY:   .dsb 1   ; Game Over popup cursor Y ($60/$68)

                .dsb 9   ; $7DCC-$7DD4 unused

    Map_PrevMoveDir:    .dsb 1   ; Last SUCCESSFUL (allowed) movement direction on map R01 L02 D04 U08

                .dsb 8   ; $7DD6-$7DDD unused

    Pal_Data:       .dsb 32  ; $7DDE-$7DFD Holds an entire bg/sprite palette (this is the MASTER palette, what fades target, and others may source for "original" colors!)

    Level_AltLayout:    .dsb 2   ; $7DFE-$7DFF Pointer to level's "alternate" layout (when you go into bonus pipe, etc.)
    Level_AltObjects:   .dsb 2   ; $7E00-$7E01 Pointer to level's "alternate" object set (when you go into bonus pipe, etc.)

    Level_BlockGrabHitMem:  .dsb 128 ; $7E02-$7E81 Records coins and 1-ups grabbed, so they don't come back if you switch areas
...
CARD_WILD   = 8 ; UNUSED Wild card (can match any other!)
    Card_ActiveSet:     .dsb 18  ; $7E82-$7E93 Active set of N-Spade game cards
    ...
    Tile_AttrTable:     .dsb 8   ; $7E94-$7E9B

    Level_UnusedSlopesTS5:  .dsb 1   ; UNUSED; If set to 2, forces slopes to be enabled for Level_Tileset = 5 (plant infestation)
    PlantInfest_ACnt_Max:   .dsb 1   ; Always set to $1A in plant infestation levels, sets max value for animation counter

                .dsb 24  ; $7E9E-$7EB5 unused

    LevelJctBQ_Flag:    .dsb 1   ; Set to '1' while in a Big Question block area, locks horizontal scrolling
    Level_JctBackupTileset: .dsb 1   ; Level Junction tileset backup
    Level_AltTileset:   .dsb 1   ; Level's "alternate" tileset (when you go into bonus pipe, etc.)

    ; The "ORIGINAL" series are so you can switch back after going to a level's "alternate"
    Level_LayPtrOrig_AddrL: .dsb 1   ; ORIGINAL Low byte of address to tile layout
    Level_LayPtrOrig_AddrH: .dsb 1   ; ORIGINAL High byte of address to tile layout
    Level_ObjPtrOrig_AddrL: .dsb 1   ; ORIGINAL Low byte of address to object set
    Level_ObjPtrOrig_AddrH: .dsb 1   ; ORIGINAL High byte of address to object set

    Level_BG_Page1_2:   .dsb 1   ; Sets which bank the first and second page (2K / 64 8x8 tiles) of BG is using (see Level_BG_Pages1/2)

    Map_BorderAttrFromTiles:.dsb 44  ; $7EBE-$7EC8 (?) Attributes collected from map tiles that get overwritten by border FIXME SIZE UNCERTAIN

    Map_Unused7EEA:     .dsb 1   ; Unused; Value retrieved from LUT at initialization of world, but never used otherwise
    Map_Objects_Y:      .dsb 14  ; $7EEB-$7EF8, Y coordinate of all map objects
    Map_Objects_XLo:    .dsb 14  ; $7EF9-$7F06, X coordinate lo byte of all map objects
    Map_Objects_XHi:    .dsb 14  ; $7F07-$7F14, X coordinate hi byte of all map objects
...
MAPOBJ_TOTAL        = $0E   ; Total POSSIBLE map objects
    Map_Objects_IDs:    .dsb 14  ; $7F15-$7F22

    Map_SprRAMOffDistr: .dsb 1   ; A free running counter on the map only which distributes Sprite_RAM offsets to ensure visibility
    ...
    Map_2PVsGame:       .dsb 1
                ...
                .dsb 8   ; $7F25-$7F2C unused

    Map_Airship_Dest:   .dsb 1   ; Airship travel destination; 6 X/Y map coordinates defined per world, after that it just sits still
    THouse_OpenByID:    .dsb 16  ; $7F2E-$7F3D UNUSED would keep track of chests opened for a given Toad House ID (THouse_ID)
    StatusBar_PMT:      .dsb 8   ; $7F3E-$7F45, tiles that currently make up the power meter >>>>>>[P]
    StatusBar_CoinH:    .dsb 1   ; Status bar tile for coin MSD
    StatusBar_CoinL:    .dsb 1   ; Status bar tile for coin LSD
    StatusBar_LivesH:   .dsb 1   ; Status bar tile for lives MSD
    StatusBar_LivesL:   .dsb 1   ; Status bar tile for lives LSD
    StatusBar_Score:    .dsb 6   ; $7F4A-$7F4F Status bar tiles for score
    StatusBar_Time:     .dsb 3   ; $7F50-$7F52 Status bar tiles for time remaining
    Map_MusicBox_Cnt:   .dsb 1   ; Number of turns remaining until hammer brothers wake up (>= 1 and they're be asleep on the map)
    ...
    Level_JctYLHStart:  .dsb 16  ; $7F54-$7F63 Array of Y / YHi starts
    Level_JctXLHStart:  .dsb 16  ; $7F64-$7F73 Array of X / XHi starts

    Object_TileFeet2:   .dsb 1   ; ? Difference against Object_TileFeet?
    Object_TileWall2:   .dsb 1   ; ? Difference against Object_TileWall?

    ObjTile_DetYHi:     .dsb 1   ; Object tile detect Y Hi
    ObjTile_DetYLo:     .dsb 1   ; Object tile detect Y Lo
    ObjTile_DetXHi:     .dsb 1   ; Object tile detect X Hi
    ObjTile_DetXLo:     .dsb 1   ; Object tile detect X Lo

    Bubble_Cnt:     .dsb 3   ; $7F7A-$7F7C Bubble counter value (0 = no bubble)
...
; an unused feature (that an object could respond to a splashdown)
    WatrHit_IsSetFlag:  .dsb 1   ; Set when Object_WatrHit* values are stored (but never cleared, so only once!)
    Bubble_YHi:     .dsb 3   ; $7F7E-$7F80 Water Bubble Y Hi
    Object_WatrHitYHi:  .dsb 1   ; Y Hi of object that just hit water
    Bubble_Y:       .dsb 3   ; $7F82-$7F84 Water Bubble Y
    Object_WatrHitY:    .dsb 1   ; Y of object that just hit water
    Bubble_XHi:     .dsb 3   ; $7F86-$7F88 Water Bubble X Hi
    Object_WatrHitXHi:  .dsb 1   ; X Hi of object that just hit water
    Bubble_X:       .dsb 3   ; $7F8A-$7F8C Water Bubble X
    Object_WatrHitX:    .dsb 1   ; X of object that just hit water

    Splash_Counter:     .dsb 3   ; $7F8E-$7F90 Water splash counter
    Splash_Y:       .dsb 3   ; $7F91-$7F93 Water splash X
    Splash_X:       .dsb 3   ; $7F94-$7F96 Water splash Y
    Splash_NoScrollY:   .dsb 3   ; $7F97-$7F99 If set, flags this water splash to not display sprite Y as relative to screen scroll

    BrickBust_En:       .dsb 3   ; $7F9A-$7F9C Brick bust "Enable" (0 = disable, 2 = brick debris, anything else = "poof" away)
    BrickBust_YUpr:     .dsb 3   ; $7F9D-$7F9F Brick bust upper chunks Y
    BrickBust_X:        .dsb 3   ; $7FA0-$7FA2 Brick bust base X
    BrickBust_YVel:     .dsb 3   ; $7FA3-$7FA5 Brick bust Y velocity
    BrickBust_XDist:    .dsb 3   ; $7FA6-$7FA8 Brick bust X split
    BrickBust_YLwr:     .dsb 3   ; $7FA9-$7FAB Brick bust lower chunks Y
    BrickBust_HEn:      .dsb 3   ; $7FAC-$7FAE Bits to hide chunks (Bit 0 = Right, 1 = Left, 2 = Lower, 3 = Upper) OR poof counter

                .dsb 3   ; $7FAF-$7FB1 unused

    CoinPUp_State:      .dsb 4   ; $7FB2-$7FB5 State of up to 4 "Power Up" coins (i.e. coins that come out of ? blocks and bricks)
    CoinPUp_Y:      .dsb 4   ; $7FB6-$7FB9 Y of "Power Up" coins
    CoinPUp_X:      .dsb 4   ; $7FBA-$7FBD X of "Power Up" coins
    CoinPUp_YVel:       .dsb 4   ; $7FBE-$7FC1 Y velocity of "Power Up" coins
    CoinPUp_Counter:    .dsb 4   ; $7FC2-$7FC5 Counter used by "Power Up" coins
...
SOBJ_LASER      = $15   ; Laser
SOBJ_POOF       = $16   ; Poof
    SpecialObj_ID:      .dsb 8   ; $7FC6-$7FCD Special object spawn event IDs

                .dsb 2   ; $7FCE-$7FCF unused

    Objects_Var3:       .dsb 5   ; $7FD0-$7FD4 Generic variable 3 for objects SLOT 0 - 4 ONLY

    SpecialObj_YHi:     .dsb 8   ; $7FD5-$7FDC Special object Y high coordinate

                .dsb 2   ; $7FDD-$7FDE unused

    Objects_LastTile:   .dsb 8   ; $7FDF-$7FE6 Last tile this object detected

    Objects_SprAttr:    .dsb 8   ; $7FE7-$7FEE Object sprite attributes (only uses bit 6 for H-Flip and bits 0-1 for palette)
    Objects_UseShortHTest:  .dsb 8   ; $7FEF-$7FF6 If set, object will use a short horizontal test to determine if it is off-screen
...
; After the wand is returned ONLY
    CineKing_WandState:     .dsb 1   ; Wand state; 0 = falling, 1 = spinning, 2 = held
    CineKing_WandFrame:     .dsb 1   ; Wand frame; 0 to 7
    CineKing_ToadFrame:     .dsb 1   ; Toad's frame
    CineKing_DiagHi:        .dsb 1   ; Text high address value

                    .dsb 3   ; $7A05-$7A07 unused in this context

    CineKing_TimerT:        .dsb 1   ; Cheering Toad animation Timer
    CineKing_Timer3:        .dsb 1   ; Timer decremented every 4 ticks (does not appear to be used!)

                    .dsb 2   ; $7A0A-$7A0B unused in this context

    CineKing_WandX:         .dsb 1   ; Wand X position
    CineKing_WandY:         .dsb 1   ; Wand Y position
    CineKing_WandXVel:      .dsb 1   ; Wand X velocity (4.4FP)
    CineKing_WandYVel:      .dsb 1   ; Wand Y velocity (4.4FP)
